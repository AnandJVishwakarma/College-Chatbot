...
function setBotResponse(response) {
    ...
    setTimeout(function() {
        ...
        if (response.length < 1) {
            ...
            var fallbackMsg = 'I couldn\'t get that. Let\' try something else!';
            ...
            for (i = 0; i < response.length; i++) {
                ...
                if (response[i].hasOwnProperty("custom")) {
                    ...
                    //check if the custom payload type is "link_attachment"
                    if (response[i].custom.payload == "link_attachment") {
                        renderlinkAttachment(response[i]);
                        return;
                    }
...
function renderPdfAttachment(data) {
    pdf_url = data.custom.data.url;
    pdf_title = data.custom.data.title;
    pdf_attachment =
...
function renderlinkAttachment(data) {
    link_url = data.custom.url;
    link_attachment =
        '<div class="link_attachment">' +
        '<a href="' + link_url + '" target="_blank"></a>' +
        '</div>' 
    $(".chats").append(link_attachment);
    scrollToBottomOfResults();
